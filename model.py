...
gcn = GraphConvLayer(device, gcn_dim, edge_dim, dep_embed_dim)

# Synthetic input data
edge_feature = torch.rand(batch_size, num_nodes, num_nodes, edge_dim, device=device)
weight_adj = torch.rand(batch_size, num_nodes, num_nodes, edge_dim, device=device)
gcn_inputs = torch.rand(batch_size, num_nodes, gcn_dim, device=device)
self_loop = torch.eye(num_nodes, num_nodes, device=device).unsqueeze(0).unsqueeze(1).expand(batch_size, edge_dim, -1, -1)
