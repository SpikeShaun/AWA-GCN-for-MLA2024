...
from models.plm_models import EMCGCN, EMCGCN_ABFF #, EMCGCN_ABFF_Conv
from models.emcgcn_old import EMCGCN_old
...
# 定义参数
train_file = './pdata/train.json'
valid_file = './pdata/test.json'
...
if use_aspect_class:
    ...
    num_labels = 4
MODEL = EMCGCN
model = MODEL.from_pretrained(pretrained_path, num_labels=num_labels)
...
train_dataloader, valid_dataloader, test_dataloader = make_emc_plm_dataloader(
...
# 评估
model_save_path = os.path.join(save_path, model_name)
trainer.model = MODEL.from_pretrained(model_save_path)
